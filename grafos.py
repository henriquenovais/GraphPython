...
def dijkstra(G, inicial, final=None, weight='weight'):
    ...
    for inicio in inicial:
        if inicio not in G:
            raise Exception("O vertice inicial não foi encontrado")
        visao[inicio] = 0
    ...
    try:
        ...
        raise Exception("Nao ha caminho para esse vertice")
...
def bellman_ford(G, inicial, final=None, weight='weight'):
    ...
    for s in [inicial]:
        if s not in G:
            raise Exception("O objeto não esta em G")
    ...
    try:
        ...
        raise Exception("Não há caminho")
...
if tipo_gr == 'imp':
    ...
    while done == False:
        ...
        if menu == "plt":
            ...
            print("Digite 'fld' para calcular a matriz de menos caminhos(Algoritmo de Floyd-Warshall)")
            ...
            if test_menu == "tar":
                ...
                res = dijkstra(g, str(vertice_i), str(vertice_f), 'weight')
                print(res)
            elif test_menu == "blm":
                print("Digite o vertice inicial.")
                vertice_i = input("Nome do vertice: ")
                print("Digite o vertice final.")
                vertice_f = input("Nome do vertice: ")
                res = bellman_ford(g, str(vertice_i), str(vertice_f),'weight')
                print(res)
            elif test_menu == "fld":
